...
const FILES_TO_PARSE = 'src/{pages,components,containers}/**/!(*.test).js'
...
for (const locale of appLocales) {
  ...
  const translationFileName = `src/lang/${locale}.json`
  try {
  ...
  try {
    ...
    for (const message of messages) {
      for (const locale of appLocales) {
        ...
        const newMsg = locale === DEFAULT_LOCALE ? message.defaultMessage : `###${message.defaultMessage}`
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir('-p', 'src/lang') // eslint-disable-line

    let localeTaskDone
    let translationFileName

    for (const locale of appLocales) {
      translationFileName = `src/lang/${locale}.json`
