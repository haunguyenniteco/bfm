...
import { formatError, isInstance as isApolloErrorInstance, createError } from 'apollo-errors'
...
const flattenNestedErrors = error =>
  (error.errors || []).reduce(
    (errors, nestedError) => [
      ...errors,
      ...[duplicateError(nestedError, ['errors']), ...flattenNestedErrors(nestedError)].map(flattenedError => {
...
export const _formatError = error => {
  ...
  try {
    ...
    if (isApolloErrorInstance(originalError)) {
      ...
      const rawStack = exception.stacktrace || exception.stack
      const stack = rawStack ? stackUtil.clean(rawStack) : ''
